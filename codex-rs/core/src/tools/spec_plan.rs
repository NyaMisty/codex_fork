...
fn build_code_mode_executors(
    ...
    let code_mode_nested_tool_specs = executors
        ...
        .collect::<Vec<_>>();
    let prompt_tool_specs = executors
        .iter()
        .filter_map(|executor| {
            if !matches!(executor.exposure(), ToolExposure::Direct) {
                return None;
            }
        ...
        .collect::<Vec<_>>();
    let namespace_descriptions = code_mode_namespace_descriptions(&prompt_tool_specs);
    let mut enabled_tools =
        collect_code_mode_exec_prompt_tool_definitions(prompt_tool_specs.iter());
    enabled_tools
